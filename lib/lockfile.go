...
func BaseLockfileCreation(c *configuration.Configuration) (*lockfile, error) {
	lockfilePath := c.TargetDir + "/hmlock.json"
	lockfile, err := ReadOrCreateLockfile(lockfilePath)
	if err != nil {
		c.Logger.Error("something went wrong while parsing the lockfile, you might need to remove it manually (possibly was generated by previous version of `hm`", "err", err)
		return nil, err
	}
